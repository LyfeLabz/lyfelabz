...
export async function materializeLmsEnrollmentsFromMembership(input: {
  ...
  for (const target of targets) {
    // Idempotent: an existing enrollment (active or terminal) is preserved
    // exactly as it is - a terminal enrollment is NOT reactivated (no
    // authorized inactive->active transition), matching the certified
    // roster-sync engine's posture. A newly created enrollment is audited
    // and best-effort reconciled as a recipient of the class's published
    // assignments; see `createEnrollmentFromTrustedMembership`.
    // eslint-disable-next-line no-await-in-loop
    const outcome = await createEnrollmentFromTrustedMembership({
      classId: target.classId,
      schoolId: target.schoolId,
      studentId: uid,
      actor: { userId: uid, role: "student" },
      source: "lmsMembership",
    });
