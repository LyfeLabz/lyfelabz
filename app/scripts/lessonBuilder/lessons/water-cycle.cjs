/*
 * Water Cycle - declarative lesson build configuration.
 *
 * Sprint 19C third migration. Every path, label, signature, and
 * expected context is declared here. The generic builder engine reads
 * this file; no Water-Cycle-specific string appears in the engine.
 */
...
Canonical source: lesson-sources/lesson_water-cycle.html
Build target: v1
Regenerate: npm --prefix app run lessons:build -- --only=water-cycle --target=v1
-->
`;
...
Canonical source: lesson-sources/lesson_water-cycle.html
Build target: v2
Regenerate: npm --prefix app run lessons:build -- --only=water-cycle --target=v2
-->
`;
...
module.exports = {
  ...
  outputs: {
    ...
    v2: "app/lessons/lesson_water-cycle.html",
  },
  ...
  // Lesson-specific minimums the Water Cycle migration must satisfy on
  // every build. These live here, not in the generic engine, so no
  // other lesson is constrained by Water-Cycle-specific counts.
