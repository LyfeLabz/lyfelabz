/*
 * Earthquakes - declarative lesson build configuration.
 *
 * Sprint 19D fourth migration. Every path, label, signature, and
 * expected context is declared here. The generic builder engine reads
 * this file; no Earthquakes-specific string appears in the engine.
 */
...
Canonical source: lesson-sources/lesson_earthquakes.html
Build target: v1
Regenerate: npm --prefix app run lessons:build -- --only=earthquakes --target=v1
-->
`;
...
Canonical source: lesson-sources/lesson_earthquakes.html
Build target: v2
Regenerate: npm --prefix app run lessons:build -- --only=earthquakes --target=v2
-->
`;

module.exports = {
  slug: "earthquakes",
  canonicalSource: "lesson-sources/lesson_earthquakes.html",
  outputs: {
    v1: "lesson_earthquakes.html",
    v2: "app/lessons/lesson_earthquakes.html",
  },
  ...
  // Lesson-specific minimums the Earthquakes migration must satisfy on
  // every build. These live here, not in the generic engine, so no
  // other lesson is constrained by Earthquakes-specific counts.
