...
// Sprint 13H: unit tests for the `assignmentsPublish` wire. The wire is
...
import { createAssignmentsPublishCallable } from "./publish-wire";
...
describe("createAssignmentsPublishCallable", () => {
  test("invokes the assignmentsPublish callable with the assignment id", async () => {
    callableResponse = {
      assignmentId: "assign-1",
      status: "published",
      alreadyPublished: false,
    };
    const callable = createAssignmentsPublishCallable(FAKE_FUNCTIONS);
    const result = await callable({ assignmentId: "assign-1" });
    expect(callableInvocations).toEqual([
      { name: "assignmentsPublish", payload: { assignmentId: "assign-1" } },
    ]);
  ...
  test("rejects when the callable response shape is malformed", async () => {
    callableResponse = { assignmentId: "assign-1", status: "draft" };
    const callable = createAssignmentsPublishCallable(FAKE_FUNCTIONS);
