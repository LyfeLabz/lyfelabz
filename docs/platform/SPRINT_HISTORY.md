...
## Sprint 13C: Persistent Teacher Assignment Enumeration and Detail Access
...
Sprint 13C adds one narrowly bounded, callable-based teacher assignment enumeration path so that an authenticated teacher's existing published and closed assignments are restored after a full page reload and the certified Sprint 13B `View summary` affordance is available without republishing. The new `assignmentsTeacherList` callable authenticates through `requireDistrictContext`, requires an active-teacher role, queries `assignments` by `teacherId`, `schoolId`, and `status in ["published", "closed"]`, resolves `className` through a teacher-authorized read of the referenced class, and returns only the minimal projection `{ assignmentId, lessonSlug, title, classId, className, status }`. No student, recipient, attempt, session, or raw assessment data is projected. The Sprint 12E Slice 1 aggregate-only boundary, the Sprint 13B Assignment Detail surface, and the four-item Teacher Workspace navigation are preserved unchanged. No Firestore Rules change, no composite index, no LMS behavior, no Google Classroom behavior, no notification system, no browser persistence, no realtime listener, no polling, and no deployment occurred. No commit was created. See `docs/platform/SPRINT_13C_COMPLETION_REPORT.md`.

### Sprint 13C multiple-assignment remediation (2026-07-17)

The initial Sprint 13C implementation preserved every hydrated assignment in the session-scoped registry but the Curriculum surface still stored a single `assignmentId` per `lessonSlug`. When a teacher had more than one concurrent assignment for the same lesson (per-class copies, republication cycles), the visible `View summary` control resolved to only one of them, effectively hiding the others behind an arbitrary selection. That arbitrary selection is now removed. Assignments are grouped by `lessonSlug` and deduplicated by canonical `assignmentId` inside a pure grouping helper. Lessons with exactly one registered assignment continue to render the existing `View summary` control that opens that assignment directly. Lessons with two or more registered assignments render a `View summaries` control that opens a compact, calm, deterministic selection interface built on the existing `shell-assign-overlay` / `shell-assign-dialog` styling. Each choice is a native `<button>` labelled `${className} · ${statusLabel}` (never an id, never any student, recipient, attempt, or session data) and is ordered by class name ascending, then by status (`published`, `closed`, `draft`), then by title, then by `assignmentId` as the final tie-breaker. Selecting a choice invokes the certified entry-point opener with the exact assignment id. Publish-time additions update the visible control immediately without requiring a reload. Reload hydration restores every choice through the same registry. No arbitrary assignment-selection behavior remains. No backend callable, authorization, query, response shape, Firestore Rules, index, LMS, Google Classroom, notification, browser persistence, realtime listener, polling, or deployment change was made. No commit was created. See `docs/platform/SPRINT_13C_COMPLETION_REPORT.md` §27.

## Sprint 13D: Teacher Assignment Status Management Foundation (Close Assignment)

Dates: 2026-07-17. Status: CONDITIONALLY CERTIFIED pending resolution of the pre-existing `curriculum/curriculumManifest.test.ts` baseline failure.

Sprint 13D introduces the first teacher assignment lifecycle action on the certified Sprint 13B Assignment Detail surface: closing a published assignment. The canonical `assignmentsClose` callable already existed in the assignments domain (authenticated through `requireDistrictContext`, restricted to the active-teacher role, enforcing owning-teacher and same-school ownership, transitioning `assignments/{assignmentId}.status` from `published` to `closed` through the certified `assignmentCloseDocRef` typed write, and writing one canonical `assignments.closed` audit event via `writeAuditEvent`), so Option A of the sprint brief was selected: reuse it exactly, no new callable and no additive extension. Client integration adds a narrow entry-point wire (`close-wire.ts`) that keeps the pure detail surface free of `firebase/*` imports, extends the surface with a secondary `Close assignment` action rendered only when status is `published` and the callable seam is wired, a calm accessible confirmation dialog reusing the existing `.shell-assign-overlay` / `.shell-assign-dialog` visual language with the exact prompt and buttons the brief prescribes, immediate in-place transition of the header to `Closed` plus a non-interactive `Assignment closed` label on success, calm error handling on failure (Published state preserved, generic teacher-facing `role="alert"` message, no Firestore or callable vocabulary leaked), and an `onStatusChange` callback that re-registers the updated metadata into the Sprint 13B / 13C session-scoped registry so the Curriculum selection interface reflects the new status without a page reload. Nine targeted detail-surface tests, four targeted close-wire tests, and the existing eleven backend tests all pass. Full Cloud Functions regression: 872 of 872. Full application suite: 341 of 342 (the sole remaining failure is the pre-existing `curriculum/curriculumManifest.test.ts` baseline drift, unchanged from Sprint 13B / 13C). Lint, typecheck, and production build pass. Firestore Rules unchanged, indexes unchanged, LMS unchanged, Google Classroom unchanged, browser persistence unchanged, realtime listeners unchanged, polling unchanged. No deployment. No commit. Zero em dashes across the completion report and this entry.
...
- `app/src/assignments/detail/detail.ts` (Close assignment action, confirmation dialog, closed-state label, pending / error handling; no firebase imports).
- `app/src/assignments/detail/detail.test.ts` (nine appended lifecycle tests).
- `app/src/index.ts` (per-session `assignmentClose` binding; wired into `renderAssignmentDetail`).
- `docs/platform/SPRINT_HISTORY.md` (this Sprint 13D entry).
...
## Sprint 13F: Persistent Draft Assignment Discovery (2026-07-17)

Bounded slice that lets a teacher rediscover a draft assignment after signing out and back in. Extends `assignmentsTeacherList` backward-compatibly with a single optional request field `includeDrafts`; when the flag is `true` the caller receives draft records the caller owns in the caller's own school in addition to `published` and `closed` records. Client hydration opts in on every call so the Sprint 13B/13C session-scoped registry restores drafts alongside published and closed assignments. Curriculum surfaces a new `View drafts` control on lessons whose entire assignment set is draft; single-draft lessons open directly, multi-draft lessons reuse the Sprint 13C selection interface. Mixed sets keep the Sprint 13C `View summary` / `View summaries` behavior unchanged so the published workflow is preserved. Assignment Detail renders the `Draft` status and a calm non-interactive `Draft assignment` lifecycle label; Close and Reopen are never exposed for drafts. Authorization, ownership, district scoping, and the aggregate-only confidentiality boundary are unchanged. No new callable was introduced. No archive, edit, delete, duplicate, bulk operation, publishing, Firestore Rules relaxation, composite index, schema change, LMS side effect, browser persistence, realtime listener, or polling was introduced.
...
- `platform/functions/src/assignments/assignments-teacher-list.ts` (additive `includeDrafts` request flag; widened status filter and returned union when opted in).
- `platform/functions/src/assignments/assignments-teacher-list.test.ts` (seven appended Sprint 13F tests covering default behavior, opt-in behavior, owned-draft return, cross-owner and cross-district isolation, and preservation of published + closed).
- `app/src/assignments/detail/hydrate.ts` (parser now accepts `draft` in the returned status union).
- `app/src/assignments/detail/hydrate-wire.ts` (callable invocation now sends `{ includeDrafts: true }`).
- `app/src/assignments/detail/hydrate.test.ts` (parser and registry hydration tests for `draft`).
- `app/src/assignments/detail/detail.ts` (draft branch renders the `Draft assignment` lifecycle label instead of Close or Reopen).
- `app/src/assignments/detail/detail.test.ts` (four appended Sprint 13F tests for draft status label, draft lifecycle label, absence of Close and Reopen, and published-workflow regression guard).
- `app/src/shell/surfaces/curriculum.ts` (draft-only branch renders `View drafts`; single draft opens directly; multiple drafts reuse the Sprint 13C selection interface; mixed and published-only paths unchanged).
- `app/src/shell/shell.test.ts` (six appended Sprint 13F curriculum tests: single draft, multiple drafts, deterministic ordering, published-only regression, mixed preservation, closed-only preservation).
- `docs/platform/SPRINT_HISTORY.md` (this Sprint 13F entry).

### Certification

Sprint 13F is CONDITIONALLY CERTIFIED. Implementation is complete, targeted backend and client tests pass, full Cloud Functions regression passes (890 / 890), lint / typecheck / build pass, and only the pre-existing `curriculum/curriculumManifest.test.ts` baseline failure remains outstanding in the full application suite. Final certification is conditional pending resolution or formal acceptance of that baseline failure.

## Sprint 13G: Draft Editing Foundation (2026-07-17)

Smallest bounded editing slice on top of Sprint 13F draft discovery. Adds an inline `Edit draft` action + lightweight editor to the Assignment Detail surface for `draft`-status assignments and wires it to the already-certified `assignmentsUpdateDraft` callable (Option A: reuse; no new callable, no callable extension, no backward-compatibility shim). The editor exposes a single narrow field (`title`); Save invokes the callable exactly once with `{ assignmentId, title? }` (title omitted when unchanged so the callable's idempotent path is exercised), updates the Detail header immediately, and re-registers the updated metadata through the existing session-scoped `assignmentDetailRegistry` via `onStatusChange` so Curriculum reflects the new title on next mount without a page reload or a callable roundtrip. Cancel closes the editor without invoking the callable. Validation blocks an empty title client-side; the callable defensively rejects `assignments.invalidTitle`. Callable failure surfaces a calm generic banner; the header is preserved and the editor stays open for retry. Authorization is unchanged (active teacher, district context, canonical ownership gate, draft-only status guard); immutable fields (ownership, school, district, class, assessment, recipients, submissions, attempts, sessions, summaries, status, createdAt) remain immutable, verified by a dedicated backend test that injects them on the payload and asserts they never appear in the write. Published, closed, and archived assignments never expose the editor. No new callable, no schema change, no Firestore Rules relaxation, no composite index, no LMS side effect, no browser persistence, no realtime listener, and no polling was introduced.

### Files created

- `app/src/assignments/detail/update-wire.ts`.
- `app/src/assignments/detail/update-wire.test.ts` (five wire tests).
- `docs/platform/SPRINT_13G_COMPLETION_REPORT.md`.

### Files modified

- `app/src/assignments/detail/types.ts` (added `AssignmentsUpdateDraftInput`, `AssignmentsUpdateDraftResult`, and `AssignmentsUpdateDraftCallable` seam types).
- `app/src/assignments/detail/detail.ts` (added `updateDraftCallable` optional dep, `Edit draft` action rendered on the draft branch when the seam is wired, inline editor with title input plus Cancel and Save controls, `EditUiState` state machine, and the `renderDraftEditor` helper).
- `app/src/assignments/detail/detail.test.ts` (eleven appended Sprint 13G client tests).
- `app/src/index.ts` (wired the `createAssignmentsUpdateDraftCallable` factory per active-teacher session; cleared on any non-teacher bootstrap outcome).
- `platform/functions/src/assignments/assignments-update-draft.test.ts` (three appended backend tests: closed rejected, archived rejected, immutable-fields ignored).
- `docs/platform/SPRINT_HISTORY.md` (this Sprint 13G entry).

### Certification

Sprint 13G is CONDITIONALLY CERTIFIED. Implementation is complete, targeted backend + wire + detail tests pass (18 / 18, 5 / 5, 58 / 58), full Cloud Functions regression passes (893 / 893), lint / typecheck / build pass, and only the pre-existing `curriculum/curriculumManifest.test.ts` baseline failure remains outstanding in the full application suite. Final certification is conditional pending resolution or formal acceptance of that baseline failure. No deployment. No commit.

### Sprint 13G Scope-Completion Remediation (2026-07-17)

A follow-up remediation reconciled the Sprint 13G draft editor with the actual whitelist of the certified `assignmentsUpdateDraft` callable. Inspection confirmed the callable accepts `title`, `instructions`, `lessonSlug`, `lessonVersion`, `mode`, `windowClosesAt`, and `availableAt` per Data Model §3.6 and §7.6. `instructions` was implemented as the second editable field (a labeled `<textarea>`, `maxLength=4000`, trim-to-compare, sent only when the trimmed value differs from the stored value and is non-empty; whitespace-only edits are treated as unchanged because the callable rejects the empty string). Availability windows (`availableAt`, `windowClosesAt`) and the lesson-revision reference (`lessonSlug`, `lessonVersion`, `mode`) were classified as schema-supported but not safely editable in this slice: no existing teacher-facing date/time control convention exists in the client, no client-side lesson-revision selector exists, and the creation flow hardcodes `lessonVersion: "v1"` and `mode: "classroom"`. The certified `assignmentsUpdateDraft` callable was reused unchanged. `assignmentsTeacherList` gained an additive optional `instructions` projection so a saved draft returns to the editor with the current value prefilled after a full page reload; the hydrate parser and `AssignmentDetailMetadata` shape gained the matching optional field. No new callable was introduced, no callable request contract changed, no Firestore Rules were relaxed, no schema field was added, no composite index was introduced. Full Cloud Functions regression: 40 suites, 896 / 896 pass. Full application suite: 399 / 400 pass; the single remaining failure is the pre-existing `curriculum/curriculumManifest.test.ts` baseline. Lint, typecheck, and build clean. Zero em dashes across every modified file. No deployment. No commit. See `docs/platform/SPRINT_13G_COMPLETION_REPORT.md` §18.

### Sprint 13F Documentation and UX Reconciliation (2026-07-17)

A documentation-and-UX reconciliation pass followed Sprint 13F implementation. `docs/platform/LYFELABZ_CLOUD_FUNCTION_CHARTER.md` Appendix A gained the canonical `assignmentsTeacherList` entry (immediately before `assignmentsArchive`) capturing the Sprint 13C authorization gate, the response projection, and the Sprint 13F additive request field `includeDrafts` (default excludes drafts; `true` also returns owned drafts in the caller's school). `docs/platform/ASSESSMENT_IMPLEMENTATION_CONTRACT.md` gained §34 "Sprint 13F Reconciliations (Teacher Draft Discoverability)" recording that draft discovery is teacher-visible only, creates no recipient / session / attempt / summary / rollup state, does not affect the Sprint 13A summary surface, and does not replace `assignmentsPublish` as the transition that exposes an assignment to students. `app/src/assignments/detail/detail.ts` now renders a calm informational panel (`Assignment results`; body: `Assignment results will appear after this draft is published and students begin submitting work.`) in place of the Sprint 13A summary card for a draft; the summary callable is never invoked for a draft. Published and closed assignments continue to compose the Sprint 13A summary card unchanged. Four targeted Detail tests were appended. No backend callable change, no Firestore Rules change, no composite index, no schema change, no LMS behavior, no Google Classroom behavior, no browser persistence, no realtime listener, and no polling was introduced. Zero em dashes across the modified files. No deployment. No commit. See `docs/platform/SPRINT_13F_COMPLETION_REPORT.md` §12A.

## Sprint 13H: Draft Publication Workflow (2026-07-17)

Smallest bounded slice on top of Sprint 13G draft editing. Adds a `Publish assignment` action to the Assignment Detail surface for `draft`-status assignments (rendered alongside the Sprint 13G `Edit draft` action) and wires it to the already-certified `assignmentsPublish` callable (Option A: reuse; no new callable, no callable extension, no backward-compatibility shim). Selecting the action opens a canonical confirmation dialog (`Publish this assignment? Students in the frozen recipient list will be able to begin submitting work.`) mirroring the Sprint 13D / 13E dialog pattern. Cancel closes without invoking the callable. Confirm invokes the callable exactly once with `{ assignmentId }`, and on success the Detail header status transitions to `Published` immediately, the Draft-only affordances (`Edit draft`, `Publish assignment`, the Sprint 13F `Assignment results will appear after this draft is published…` informational panel) are removed, the Sprint 13D `Close assignment` lifecycle action becomes visible, the Sprint 13A Assignment Summary card is composed in place of the draft panel, and the updated metadata is re-registered into the session-scoped `assignmentDetailRegistry` via `onStatusChange` so a later navigation to Curriculum flips `View drafts` to `View summary` / `View summaries` per the existing Sprint 13C selection interface without a page reload. Callable failure surfaces a calm generic banner (`We could not publish this assignment right now. Try again in a moment.`); the header stays `Draft` and the publish action stays actionable so the teacher can retry. Authorization is unchanged (active teacher, district context, canonical ownership + school gate, `draft` -> `published` transition guard, already-`published` idempotency, and rejection of every other status per Data Model §3.6); the atomic status write plus initial recipient snapshot batch, and the recipient population read (`enrollments` filtered by frozen `classId`, defense-in-depth-filtered against frozen `schoolId` and `status === "active"`), are unchanged from the certified `assignmentsPublish` handler. Published, closed, and archived assignments never expose the publish action. No new callable, no schema change, no Firestore Rules relaxation, no composite index, no LMS side effect, no Google Classroom side effect, no notification, no browser persistence, no realtime listener, and no polling was introduced.

### Files created

- `app/src/assignments/detail/publish-wire.ts`.
...
- `docs/platform/SPRINT_13H_COMPLETION_REPORT.md`.

### Files modified

- `app/src/assignments/detail/types.ts` (added `AssignmentsPublishResult` and `AssignmentsPublishCallable` seam types).
- `app/src/assignments/detail/detail.ts` (added `publishCallable` optional dep, `PublishUiState` state machine, `Publish assignment` action rendered on the draft branch when the seam is wired, canonical confirmation dialog, success handler that transitions the header to `Published` and re-composes the Sprint 13A summary card, failure banner, and Escape/backdrop-cancel parity with the Sprint 13D / 13E dialogs).
- `app/src/assignments/detail/detail.test.ts` (eleven appended Sprint 13H client tests).
- `app/src/index.ts` (wired the `createAssignmentsPublishCallable` factory per active-teacher session; cleared on any non-teacher bootstrap outcome).
- `docs/platform/SPRINT_HISTORY.md` (this Sprint 13H entry).
