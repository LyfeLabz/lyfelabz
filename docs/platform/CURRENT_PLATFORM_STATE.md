...
- **Account lifecycle** (`status` on `users/{uid}`): `provisioned` → `pendingVerification` (teachers only) → `active` → `suspended`. `awaitingFirstSignIn` is a **roster-level** placeholder state, not a user state. Teacher suspension (`active` → `suspended`) is implemented by the `teachersSuspend` administrative callable (Phase 8G.1, hardened in Phase 8G.3); it preserves identity and all instructional data, attempts to clear claims and revoke refresh tokens as a best-effort accelerator (those Auth-side steps are idempotently repairable if they transiently fail; an already-issued ID token may briefly carry stale claims until it expires), and is enforced immediately regardless of Auth cleanup outcome because the LMS actor gate, the teacher-side Firestore Rules, and the shared authenticated read surfaces (schools, lmsProviders, assessments, assessmentRevisions - gated by an active-user check) consult authoritative `status`. Phase 8G.3 also moved the administrator authority check and the target/school context resolution INSIDE the suspension transaction (no admin TOCTOU, no stale context), and made the LMS actor gate derive authoritative tenant context (`schoolId`/`districtId`) from canonical records rather than token claims. Reinstatement and archival remain reserved states.
- **Platform administrator role transition** (`users.roleChanged`, Phase 8G.12 / 8G.12A): there is no in-product path to create the first `platformAdministrator` (every admin callable requires an existing active admin). The first administrator is established by promoting an existing active canonical **teacher** through an operator-only, dry-run-by-default bootstrap tool (`platform/functions/src/scripts/admin/bootstrap-platform-administrator.ts`) that is **not** a callable and is **not** exported from the Functions bundle. The CLI **explicitly binds** Firebase Admin to the acknowledged `--project-id` (never inferring the project from env/ADC), refuses to run if a Firestore/Auth emulator-host override is set (no emulator/production split-brain), resolves and validates the exact `[DEFAULT]` app it uses, and verifies the bound project before any read/write. In ONE Firestore transaction it transitions the canonical `role`, creates a durable server-only lineage record (`platformAdminBootstrap/initial`), and writes exactly one `users.roleChanged` audit; then it replaces custom claims (canonical shape, never merged) and revokes refresh tokens, in that order (never claims-first). It fails closed on any missing prerequisite (Firebase Auth identity, canonical `users/{uid}` and full record shape incl. email/displayName/createdAt, wrong project, disabled identity, mismatched UID/email/role/school, an already-existing administrator or lineage, absent/conflicting lineage on replay, invalid change-ticket, or a target already carrying an admin claim), never fabricates identity or user state, and is idempotently repair-replayable (gated on matching durable lineage) without emitting a second audit event. Concurrent first-admin attempts are serialized by the singleton lineage document (exactly one admin/lineage/audit). A paired, audited rollback mode demotes back to the lineage-recorded prior role (destination cannot be an arbitrary operator argument). Instructional/Classroom/LMS history of the promoted teacher is preserved; teacher-only access is unavailable while the role is `platformAdministrator`. See `PLATFORM_ADMIN_BOOTSTRAP_RUNBOOK.md`.
- **Administrator authority is authoritative, not claim-only** (Phase 8G.12 / 8G.12A): `schoolsCreate`, `teachersApproveVerification`, `teachersDenyVerification`, and `identityMigrationRunProductionInventory` verify administrator authority against BOTH the signed claim AND canonical Firestore (`users/{uid}` with `authUid === uid`, `status === "active"`, `role === "platformAdministrator"`), deriving tenant context from canonical records. The three **mutating** callables now re-validate administrator authority INSIDE the same Firestore transaction as their mutation and audit (`assertActivePlatformAdministratorInTransaction`), so an administrator demoted between the cheap pre-check and commit cannot mutate (demotion race closed); the read-only inventory revalidates at each page boundary (one page per call). A stale administrator token cannot retain authority after canonical demotion/suspension/archival/`authUid` divergence. `teachersSuspend` keeps its stronger in-transaction admin validation.
- **Administrator claim shape (current, ambiguity resolved).** A `platformAdministrator` currently carries canonical `schoolId` and `districtId` claims, exactly like other active roles (the canonical claims writer requires both as non-empty strings for every role). The historical "administrator sentinel" question in `DISTRICT_SECURITY_BOUNDARY_IMPLEMENTATION_CONTRACT.md` §6 / G-10A-3 and PDR-025h (absence-of-district/school as a sentinel, or a global administrator) is **not** implemented; no global/sentinel administrator exists in the current architecture.
...
- **Roster authority = Google Classroom** for linked classes; the LyfeLabz roster is a mirror. **Roster sync** is teacher-initiated refresh at v1 (client roster sync landed Sprint 24B); continuous/automatic sync is a reversible opt-in future capability. Opening a class makes no Google Classroom call; the teacher refreshes from Class settings ("Refresh roster from Google Classroom", `lmsClassesRefreshRoster` with `reconcileEnrollments`), which updates the membership cache used by first sign-in, enrolls already-active LyfeLabz students newly in the Classroom class, withdraws (never deletes) students who left it, and restores a student who returns only when their enrollment carries Classroom withdrawal provenance for the current link. An empty Classroom roster removes no one.
...
- **Enrollment** state vocabulary: `active`, `transferred`, `withdrawn`, `archived`. Enrollments are never deleted; history is preserved. Withdrawals record provenance forward-only (`exitSource: "lmsRosterSync"` + `exitLinkId`, or `exitSource: "teacher"`); a missing `exitSource` is unknown historical provenance, never inferred or backfilled. The only `withdrawn -> active` transition is Classroom-managed reactivation of an `lmsRosterSync` withdrawal for the class's current link (see `LYFELABZ_FIRESTORE_DATA_MODEL.md` §3.4).
