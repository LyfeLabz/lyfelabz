...
**Next logical sprint: Sprint 26 - LMS UX hardening (proposed).** Carries the Sprint 25 deferred follow-up, none of which blocked Sprint 25 closure. Primary item: proactive Google account continuity during incremental publication consent - thread the connection's stored Google account identifier into `beginOAuth` and set `login_hint` for publication intent so Google pre-selects the connected account and suppresses the account chooser, while preserving the existing post-callback `lms.identityMismatch` hard invariant. Additional items: clearer identity-mismatch UX; a reproducible OAuth cancellation-fixture strategy plus consent-flow observability so a future live B13 is achievable; resolution of the failure-path client defects 2.A/2.B and subsequent live certification of B14 - B23; and the Settings/credential-health UI polish backlog. Details and ownership in `SPRINT_25_COMPLETION_REPORT.md` §13. Publication and all Sprint 26 work must preserve the one-dialog assign workflow in `ASSIGN_EXPERIENCE.md`.
