...
**Status:** Ratified under Sprint 25 (Phase 2 preparation). Canonical.

**Date:** 2026-08-06.

**Anchor documents:** `SPRINT_25_DEFINITION.md`, `SPRINT_25_ARCHITECTURAL_BLUEPRINT.md`, `SPRINT_25_IMPLEMENTATION_PLAN.md`, `PDR_030_LMS_ASSIGNMENT_PUBLICATION.md`, `ADR_LMS_PUBLICATION_SURFACE_RECONCILIATION.md`, `LMS_INTEGRATION_ARCHITECTURE.md`, `LMS_INTEGRATION_ARCHITECTURE_AMENDMENT.md`, `ASSIGN_EXPERIENCE.md`.

**Context.** PDR-020c authorized a narrow initial LMS scope (connection lifecycle, discovery, import) and explicitly excluded assignment publication, while recording that each excluded capability remains reachable as its own subsequent sprint under the internal Phase 9 sequence in `LMS_INTEGRATION_ARCHITECTURE_AMENDMENT.md` §8, where publication is LMS Sprint D. Expansion of the initial scope is authorized by specification, never by implementation. Sprint 24B advanced Phase 9 to deliver the activation seam and roster synchronization, both production certified. Publication is the next capability in the sequence. The certified tree already contains the publication server contract: the `lmsAssignmentsPublish` callable, the reserved `lmsAssignmentPublications` collection and its rules, the additive `assignments.lmsPublicationRef` field, the reserved `lms.assignmentPublished` and `lms.publishFailed` audit vocabulary, the `lmsClassesListTopics` callable, and the declared but unrequested coursework scopes (`GOOGLE_CLASSROOM_PUBLICATION_SCOPES`). What is missing is the live upstream write (delivered in Sprint 25 Phase 1), the consent to obtain the coursework scopes, and the Assign dialog affordances. This record authorizes the consent and scope additions; the specification and blueprint authorize the rest. This record extends PDR-019 (LMS Integration Posture) and PDR-020 (LMS Phase Re-Sequencing and Initial Scope) and overrides neither. Where this record and any load-bearing sub-decision of PDR-019 appear to conflict, PDR-019 controls and this record is amended. PDR-004, PDR-005, PDR-010, PDR-013, PDR-019, PDR-020, PDR-027, and PDR-029 are preserved.

**Decision.** LyfeLabz publishes an existing LyfeLabz assignment to a linked Google Classroom course as a one-way pointer, initiated by the teacher from inside the one Assign dialog, as an optional extension of assigning. The coursework publication OAuth scopes are requested through incremental, opt-in consent at the moment the teacher first publishes, and are added to the teacher's existing connection rather than to a second connection. Publication is never a standalone workflow. This record adds only the two decisions Sprint 25 requires that PDR-019 and PDR-020 did not already settle: the coursework consent posture and the coursework scope set.

**Sub-decisions.**

- **PDR-030a. Publication is an extension of Assign, never a standalone workflow.** The single origin of every assignment record is the Assign dialog (PDR-010, PDR-019d). Publication is an optional, opt-in side effect of assigning to an LMS-linked class, configured on the class row inside that one dialog. The teacher decides where to assign; she never separately decides where to publish. Google Classroom is one delivery destination for an assignment she already made. No publish wizard, no Google Classroom settings panel, no LMS-specific dialog, and no Settings-based publish affordance may be introduced. Settings > Integrations remains account-level only, as narrowed by Sprint 24B. Any existing client wiring seam for publication is provided to the Assign surface, not surfaced in Settings, as recorded in `ADR_LMS_PUBLICATION_SURFACE_RECONCILIATION.md`.
- **PDR-030b. Coursework publication scopes.** Publication requires the Google Classroom coursework scopes (`classroom.coursework.me` and `classroom.topics.readonly`) in addition to the readonly scopes already granted for discovery and roster. `classroom.coursework.me` is the minimum-required write scope: it scopes the write to the teacher's own coursework and is preferred over any broader coursework scope. No scope beyond the coursework scopes named here is authorized by this record; a broader scope requires a superseding record.
  - **PDR-030b SUPERSEDED (2026-08-16, Sprint 25 B9 live certification) - see PDR-030g.** The write scope named above (`classroom.coursework.me`) is corrected to `classroom.coursework.students`. This is a correction of the specific Google scope string only; the design principle of PDR-030b (minimum-required, opt-in, teacher's own coursework, no broader Classroom access) is unchanged and remains in force. The original wording is retained above for historical traceability. See PDR-030g for the live evidence and the corrected scope set.
- **PDR-030c. Incremental, opt-in consent.** The coursework scopes are requested through incremental OAuth consent at the moment the teacher first publishes, and only then. Consent uses the certified connection lifecycle callables (`lmsConnectionsBegin`, `lmsConnectionsComplete`); no new connection callable is introduced. These callables and the adapter `beginOAuth` method receive a bounded additive extension to carry and record the requested scope set, expressed as a provider-neutral capability selector rather than a raw Google scope string on the client-facing contract; this is an extension of the existing lifecycle, not a new callable. The exact mechanics are in the blueprint §7 and the implementation plan Phase 2. Previously granted scopes are preserved across the incremental grant; the teacher is not asked to re-grant readonly scopes. Consent never escalates silently: it is requested only for the coursework capability and only when the teacher chooses to publish.
- **PDR-030d. Scope widening on the existing connection.** On grant, the teacher's existing connection scope set is widened. No second connection is created. The same connection identity, token reference lineage, and account identity are retained. Because `lmsConnectionsComplete` today writes a connection creation record, achieving this requires the completion path to merge granted scopes into an existing connection when one exists rather than create a duplicate; because the connection document identifier is deterministic per (teacher, provider), the completion path targets the same document by construction, and the widening is an additive update of that one document. The granted Google identity is revalidated against the existing token bundle's account identity (resolved through the server-only token store) before the scope set is widened; this revalidation is a new implementation that follows the same security principle as the certified import-time profile verification, not a reused helper, because no reusable helper for this connection-widening comparison exists in the certified tree. A mismatch is refused with a plain-language message and leaves the existing connection intact.
- **PDR-030e. Inherited invariants.** One-way publication (PDR-019d): LyfeLabz publishes to the LMS; the LMS never authors a LyfeLabz assignment. Server-only tokens (PDR-019e): coursework tokens are held server-side only and never cross the callable boundary. No new role, claim, or lifecycle field (PDR-019f). Additive schema (PDR-019g): publication populates the reserved `lmsAssignmentPublications` collection and the reserved `lmsPublicationRef` mirror; no new collection, no renamed field. Append-only audit (PDR-013): publication emits only the reserved `lms.assignmentPublished` and `lms.publishFailed` events; no new audit kind. Privacy is not widened (PDR-019k): no student PII is read or written by the publish path; audit payloads and log lines carry no student data, no Google email, and no token. Vendor-neutral core (PDR-019h): every Google concept lives in the provider adapter; the core and the client contract stay provider-neutral.
- **PDR-030f. Verification is a rollout prerequisite, not an implementation blocker.** Google OAuth verification for the coursework scopes lifts the unverified-app warning and the sensitive-scope user cap; it is a production rollout gate. Sprint 25 engineering and certification proceed in the Emulator Suite with a Google Classroom API test double, independent of the verification timeline. Rollout waits on verification; the sprint does not. The authoritative classification of the coursework scopes is Google's own current OAuth documentation, confirmed as part of the rollout prerequisite.
- **PDR-030g. Publication write scope correction (supersedes the scope string in PDR-030b).** Live Sprint 25 B9 certification against real Google Classroom disproved the `classroom.coursework.me` assumption. Observed sequence: the readonly-only connection first attempted publication and received HTTP 403 `PERMISSION_DENIED` / `ACCESS_TOKEN_SCOPE_INSUFFICIENT`; incremental consent opened; Google granted `classroom.coursework.me` and `classroom.topics.readonly` on top of the readonly scopes; `lmsConnectionsComplete` succeeded and logged `lms.connectionScopesWidened`; the automatic post-consent re-issue used the newly widened token; and real Google returned HTTP 403 `ACCESS_TOKEN_SCOPE_INSUFFICIENT` again. Token-lifecycle analysis ruled out stale `tokenRef`, stale access token, worker isolation, token-store inconsistency, and connection-metadata/token-bundle divergence; the post-consent token genuinely carried `classroom.coursework.me`. The cause is the scope itself: `classroom.coursework.me` is a caller-scoped (student-facing) scope and does not authorize a teacher creating coursework in a course they own. LyfeLabz performs teacher-side `courses.courseWork.create`, which requires `https://www.googleapis.com/auth/classroom.coursework.students`. The corrected publication scope set is exactly `classroom.coursework.students` and `classroom.topics.readonly`. Only the specific write-scope string changes; the initial readonly scope set (`classroom.courses.readonly`, `classroom.rosters.readonly`), the incremental/opt-in consent posture (PDR-030c), the single-connection widening (PDR-030d), and the minimum-required principle (PDR-030b) are unchanged. No scope beyond the two named here is authorized; a broader scope still requires its own superseding record.
- **PDR-030h. Automatic access-token refresh in the credential lifecycle.** Google Classroom OAuth access tokens expire ~1 hour after they are minted; the stored refresh token outlives them. This is normal Google behavior, and both the refresh-token material and the absolute `expiresAtEpochMs` were already persisted in the server-only token bundle since Sprint 23D. Live Sprint 25 B8 certification exposed that no production caller ever refreshed: `FirestoreLmsTokenStore.resolve()` returned the stored access token verbatim, so a valid, active connection whose access token had expired sent a dead token to `courses.courseWork.create`, and real Google returned HTTP 401 `invalid_token`. The refresh transport (`refreshAccessToken`) existed but was never invoked, `LmsProviderAdapter` exposed no refresh operation, and a scope-unchanged reconnect was ineffective (the `alreadyAuthorized` path discards the newly exchanged token). **Decision:** access-token refresh is performed automatically at credential resolution, centrally, so every Google Classroom operation benefits (course discovery, roster read, topic listing, assignment publication, and any future Classroom call). A new vendor-neutral resolver `resolveLiveCredential(tokenRef)` replaces the raw `getLmsTokenStore().resolve` at every callable that reaches upstream. Policy: a token comfortably before expiry is reused; a token within a 5-minute skew of expiry, or already expired, is refreshed; a due token with no refresh material preserves the existing explicit authorization-failure behavior (the stale token is handed on so the upstream 401 path is not masked). A refresh is credential maintenance, NOT an authorization event: it never mints OAuth state, never widens or regresses scope (the connection document remains the authoritative scope record and `scopes` / `scopesUpdatedAt` are untouched), never changes the upstream account identity, and does not emit `lms.connectionScopesWidened`. The refresh merges provider material conservatively: Google's `refresh_token` grant omits both a rotated refresh token and an authoritative scope set, so the existing refresh token and existing scopes are preserved verbatim; only the access token and `expiresAtEpochMs` are renewed. The `tokenRef` is STABLE across refresh: `FirestoreLmsTokenStore.persistRefreshedCredential` updates the existing `lmsTokenBundles/{tokenRef}` document in place inside a Firestore transaction rather than minting a new reference, so the connection pointer never rotates for ordinary maintenance and an intact connection self-heals without disconnect/reconnect. Concurrency across Functions workers is handled by a durable compare-and-swap on `expiresAtEpochMs`: the upstream network refresh happens outside the transaction, and the transaction discards an older refresh result when the stored expiry has already been advanced by a concurrent worker, so no worker overwrites a newer bundle and no refresh token is lost. Observability adds three structured events carrying only safe metadata (`lms.accessTokenRefreshStarted`, `lms.accessTokenRefreshed`, `lms.accessTokenRefreshFailed`); no access token, refresh token, authorization code, or client secret is ever logged. A refresh that cannot recover the credential (revoked/`invalid_grant` refresh token -> `lms.upstreamAuthorizationFailed`) surfaces the normalized `lms.reconnectRequired` error so the original callable reports the correct failure and never falsely marks publication success; a transient upstream failure propagates verbatim and is not treated as a reconnect requirement. The connection `status` is intentionally NOT mutated by the resolver on an unrecoverable refresh (no broad new UI health-state system is introduced by this record); `lmsClassesRefresh` maps `lms.reconnectRequired` to its existing `reconnectRequired` health verdict. This record changes credential lifecycle only; it does not touch the publication scope architecture (PDR-030g), incremental-consent architecture (PDR-030c/d), assignment lifecycle, topic selection, class-link semantics, or any UI.

**Reconciliation notes.**

- PDR-019 is preserved. Every load-bearing sub-decision of PDR-019 applies to Sprint 25 without exception; PDR-030 adds only the coursework consent posture and scope set.
- PDR-020 is preserved. The coursework scope addition is authorized here by specification, honoring PDR-020's rule that scope expansion is never authorized by implementation.
- PDR-027 is preserved. Google Classroom publication continues to create one coursework pointer per linked class and never adds recipients silently on the LyfeLabz side.
- PDR-029 is preserved. Publication does not alter the frozen recipient population; a coursework pointer is a delivery side effect, not a recipient-population event.

**Anti-decisions.** PDR-030 does not request the coursework scopes up front at connect time. PDR-030 does not create a second connection, a second connection collection, a publication-specific token store, a Google-specific client authority flag, or a parallel OAuth workflow. PDR-030 does not expose publication as its own surface (no publish button in Settings, no Google Classroom panel). PDR-030 does not authorize any scope beyond `classroom.coursework.me` and `classroom.topics.readonly`. PDR-030 does not authorize the client to see, hold, or forward any OAuth token. PDR-030 does not introduce a new audit kind, a new role, a new claim, or a new lifecycle field. PDR-030 does not tie Sprint 25 engineering or certification to the external Google verification timeline. PDR-030 does not commit any code, Firestore Rules, index, schema, dependency, or configuration change.

**Consequences.** The teacher gains an optional delivery destination without a new workflow, a new surface, or a new mental model. The platform reuses its certified publish callable, records, rules, audit vocabulary, topics callable, connection lifecycle, and token store; the new engineering surface is narrow. Scope growth is explicit, opt-in, minimum-required, and recorded. Account custody remains singular: one connection per teacher per provider. Engineering is not held hostage to the Google verification timeline. Limitations: broad production rollout waits on Google OAuth verification of the coursework scopes (an accepted operational constraint tracked separately from the sprint); a teacher who declines the incremental consent cannot publish but can still assign (activation without publication is a supported state); multi-account teachers hold multiple LyfeLabz identities and multiple connections, a pre-existing PDR-004 concern unchanged by this record.

**Alternatives considered.** Request the coursework scopes up front at connect time: rejected; it violates minimum-required-scope and opt-in-per-action posture. Create a second connection for the coursework scopes: rejected; it duplicates account custody, complicates disconnect and rotation, and confuses the account-level Settings surface. Expose publication as its own surface: rejected; it creates a second assignment-adjacent workflow and contradicts PDR-019d and the one Assign dialog. Authorize the scope addition by implementation without a decision record: rejected; scope expansion is authorized by specification, never by implementation, per PDR-020. Defer publication until Google OAuth verification completes: rejected as a sequencing constraint on engineering; verification gates rollout only.

**Future Reconsideration Criteria.** PDR-030a is reconsidered only through a record that redefines the single-Assign-dialog origin (PDR-010, PDR-019d); it does not become negotiable through implementation or through the location of a wiring seam. PDR-030b is reconsidered only when a publication capability requires a scope beyond the coursework scopes named here; any broader scope is authored as a superseding record with its own verification assessment. PDR-030c and PDR-030d are reconsidered only if the certified OAuth connection lifecycle or the server-only token boundary is itself redefined. PDR-030e is reconsidered only through the formal-review path required for the parent PDR-019 sub-decisions it inherits. PDR-030f is reconsidered only if the platform adopts a policy that ties engineering milestones to external verification timelines, which it does not today. Reconsideration of any PDR-030 sub-decision requires the same level of scrutiny as PDR-019 and PDR-020.
...
- 2026-08-06 - PDR-030 (LMS Assignment Publication Consent and Scope) ratified under Sprint 25 (Phase 2 preparation). Authorizes the incremental, opt-in Google Classroom coursework consent (`classroom.coursework.me`, `classroom.topics.readonly`) requested at first publish through the certified `lmsConnectionsBegin` / `lmsConnectionsComplete` lifecycle, and the widening of the teacher's single existing connection rather than the creation of a second connection. Publication is recorded as an extension of the one Assign dialog, never a standalone or Settings-based workflow. Inherits every load-bearing PDR-019 invariant (one-way publication, server-only tokens, additive schema, append-only audit, unwidened privacy, vendor-neutral core). No canonical prior PDR is amended. The standalone `PDR_030_LMS_ASSIGNMENT_PUBLICATION.md` is retained and its status updated to point here as the ratified record.
- 2026-08-16 - PDR-030g (Publication write scope correction) added under Sprint 25 B9 live certification. Supersedes the specific write-scope string in PDR-030b: the publication write scope is corrected from `classroom.coursework.me` to `classroom.coursework.students` because LyfeLabz performs teacher-side `courses.courseWork.create`, which the caller-scoped `.me` does not authorize (proven by real Google returning `ACCESS_TOKEN_SCOPE_INSUFFICIENT` after `.me` was granted). The corrected publication scope set is exactly `classroom.coursework.students` and `classroom.topics.readonly`. The initial readonly scope set, the incremental/opt-in consent posture (PDR-030c), the single-connection widening (PDR-030d), and the minimum-required principle (PDR-030b) are unchanged. PDR-030b's original wording is retained for historical traceability with an inline superseded note. Corrects `GOOGLE_CLASSROOM_PUBLICATION_SCOPES` in the Google Classroom adapter and the associated OAuth/scope tests; prior B9 certification logs are preserved unchanged.
- 2026-08-16 - PDR-030h (Automatic access-token refresh in the credential lifecycle) added under Sprint 25 B8 live certification. Live B8 exposed that no production caller refreshed an expired Google Classroom access token: an active, correctly scoped connection whose ~1h access token had expired sent a dead token to `courses.courseWork.create` and real Google returned HTTP 401 `invalid_token`, producing a `failed` publication with `lms.upstreamAuthorizationFailed`. The refresh transport and both the refresh token and `expiresAtEpochMs` were already persisted since Sprint 23D but never used. Introduces a central vendor-neutral resolver `resolveLiveCredential(tokenRef)` (new `platform/functions/src/lms/tokens/credential-resolver.ts`) that refreshes an expired/near-expiry access token (5-minute skew) before any upstream call, wired into `lmsAssignmentsPublish`, `lmsClassesListTopics`, `lmsClassesImport`, `lmsClassesDiscover`, `lmsClassesRefresh`, and the roster sync engine. Adds `LmsProviderAdapter.refreshCredential` (Google adapter maps `refreshAccessToken`) and `LmsTokenStore.persistRefreshedCredential` (Firestore transaction with a compare-and-swap on `expiresAtEpochMs`, tokenRef stable, refresh token and scopes preserved). Refresh is not an authorization event: no OAuth state, no scope change, no `scopesUpdatedAt` change, no `lms.connectionScopesWidened`, no upstream-identity change. Adds `lms.accessTokenRefreshStarted` / `lms.accessTokenRefreshed` / `lms.accessTokenRefreshFailed` structured logs (no token material). An unrecoverable refresh (`invalid_grant`) normalizes to `lms.reconnectRequired`; the resolver does not mutate connection status. Credential lifecycle only; the publication scope set (PDR-030g), consent posture (PDR-030c/d), and every UI/lifecycle surface are unchanged.
- 2026-08-17 - Sprint 25 closed COMPLETE WITH DOCUMENTED LIMITATION. Canonical record: `SPRINT_25_COMPLETION_REPORT.md`. The decisive Google Classroom publication path was certified end to end against real Google (B9/B10 genuine incremental consent; B8/B11/B12 real coursework filed under the selected topic and pointing at the LyfeLabz URL). PDR-030g (scope correction to `classroom.coursework.students`) and PDR-030h (automatic access-token refresh) were both discovered and re-certified live. B13 (cancel-consent) is PASS WITH LIMITATION per `SPRINT_25_B13_ARCHITECTURE_REASSESSMENT_AND_CLOSURE.md`: the live cancel branch is fixture-limited by Google's `include_granted_scopes` accumulated-grant behavior and is instead covered by deterministic unit tests; identity continuity is protected reactively by the `lms.identityMismatch` invariant. Deferred follow-up (proposed Sprint 26 LMS UX hardening): proactive account continuity via `login_hint`, preserving the existing `lms.identityMismatch` hard invariant. No PDR is amended by this closure; production rollout (level E) remains gated per PDR-030f.
