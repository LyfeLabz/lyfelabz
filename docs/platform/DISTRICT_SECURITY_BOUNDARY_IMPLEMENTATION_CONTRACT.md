...
- **G-10A-3. Platform administrator claim shape.** The claim shape in §6 permits either omitted `districtId`/`schoolId` on administrator tokens or an explicit administrator sentinel; the certified architecture does not settle which is canonical. The implementation sprint MUST pick one and document it as an amendment to this contract. Absence-as-sentinel is preferred because it aligns with the rule "claims are only present when `status === "active"` and the role has a district".
  ...
  **AMENDMENT (Phase 8G.12 - settled).** The implemented decision is **neither** sentinel: under the current architecture a `platformAdministrator` carries canonical `schoolId` and `districtId` claims exactly like other active roles. The canonical claims writer (`shared/auth/claims.ts`) requires both as non-empty strings for every role, and the first-administrator bootstrap (`PLATFORM_ADMIN_BOOTSTRAP_RUNBOOK.md`) sets them from the target's canonical school/district. No absence-of-district/school sentinel and no global administrator model is implemented. Administrator-only callables derive tenant context from canonical `users/{uid}` / `schools/{schoolId}` rather than trusting token tenant claims (see `requireActivePlatformAdministrator`). Should a future phase introduce a genuinely cross-district administrator, this decision must be revisited with a new PDR; until then §6's sentinel language is superseded by this amendment for current behavior.
