...
**Terminology note.** LyfeLabz has two runtime modes: **Practice Mode** (client-only, no persistence) and **Classroom Mode** (server-finalized, persisted). The Assignment record's `mode` field takes exactly these two values (`practice`, `classroom`) and never `"graded"`; a `classroom`-mode Assignment record is not itself a "graded assignment," it is a Classroom Mode surface. Sprint 30A.1 narrowly amends PDR-010 (see `LYFELABZ_PLATFORM_DECISIONS.md` §PDR-010, "Terminology Amendment (Sprint 30A.1)") to authorize "Graded"/"Ungraded" as teacher-facing vocabulary strictly for Google Classroom grading configuration, carried on a separate additive field (`AssignmentRecord.classroomGrading`) that is orthogonal to `mode`. The prohibition on "graded" everywhere else PDR-010 already forbade it is unchanged.
...
Claims are written only when the user's `status` (see `PLATFORM_STATE_MACHINE.md`) is `active`. A user in any other lifecycle state (`pendingVerification`, `provisioned`, `suspended`, `archived`) is not issued new claims, and the absence of newly-issued claims is the canonical signal that no active authorization applies. Note: an already-issued, unexpired ID token may temporarily carry stale claims after a lifecycle change such as suspension; the `teachersSuspend` callable clears claims and revokes refresh tokens after the durable transaction as a best-effort accelerator, but those Auth-side steps are idempotently repairable if they transiently fail. Authorization enforcement is always against canonical Firestore state (`users/{uid}.status`), not the token content, so stale claims on an in-flight token do not grant teacher access. Every custom claims write in the platform flows through the single canonical helper defined by the Engineering Standards (PDR-017); no second write path exists.
...
**teachersSuspend** (Administrative callable). Platform Administrator only. A cheap request-boundary gate first confirms an authenticated caller carrying the `platformAdministrator` claim; the AUTHORITATIVE administrator check then runs INSIDE the durable transaction, where the caller's `users/{adminUid}` record must be an `active` `platformAdministrator` with a consistent `authUid`. Accepts only `targetUid`; every authoritative field is resolved server-side, transactionally. Transitions a target teacher from `active` to `suspended`. The single Firestore transaction reads and validates the administrator record, then the target record (existence, `authUid` matches `targetUid`, `role: "teacher"`, `schoolId` present, source status), then resolves the target's `districtId` from the canonical `schools/{schoolId}` document, and finally writes the status change and exactly one `users.suspended` audit event atomically (the audit records `previousStatus`, `role`, and the transactionally observed school/district context; no email, OAuth identifier, or token reference). Because all authoritative reads and both writes are in one transaction, an administrator suspended/demoted concurrently, or a target whose context changes concurrently, cannot produce a suspension (no admin TOCTOU, no stale target context). After the durable boundary it clears the target's custom claims and revokes their refresh tokens; those Auth-side steps are idempotently repairable, and a replay after a revocation failure repairs Auth state without a second audit event. Idempotent under an already-`suspended` target: returns `alreadySuspended: true`, emits no second audit event, performs no second transition, and re-runs the Auth cleanup. Rejects unauthenticated, non-admin, stale/inactive-admin, teacher, and student callers, and rejects self-target, non-teacher target, missing/malformed (identity-inconsistent) target, and any source lifecycle state other than `active` or `suspended` (i.e., `provisioned`, `pendingVerification`, and `archived` are rejected; `active` triggers the live transition; `suspended` triggers the idempotent repair path). Suspension never disconnects Google Classroom, deletes LMS connections/classes/assignments, changes rosters/enrollments, or deletes the Firebase Auth identity. Enforced immediately (not only via claim clearing) because the shared LMS actor gate and the teacher-side Firestore Rules consult authoritative `status`. Reinstatement is a separate future workflow. Phase 8G.1; hardened in Phase 8G.3.
...
**assignmentsPublish** (Callable). Owning teacher only. Advances the lifecycle field on `assignments/{assignmentId}` from `draft` to `published` and stamps a server-side `publishedAt` timestamp on the same batch (Sprint 15 Slice 2; the timestamp is written exactly once on the initial `draft` -> `published` transition and is preserved across subsequent `close` and `reopen` cycles because those narrow lifecycle writes intentionally exclude the field). Modifies no other field. Rejects a cross-teacher or cross-school publish with `assignments.forbidden`, and rejects a publish from any status other than `draft` with `assignments.invalidTransition`. Emits a single `assignments.published` audit event whose payload names `{ classId, lessonSlug, lessonVersion }`. Idempotent under an already-`published` target: no second write and no second audit event are emitted and `alreadyPublished: true` is returned. Sprint 4D.
...
**assignmentsReopen** (Callable). Owning teacher only (authenticated caller resolved through `requireDistrictContext`; `role === "teacher"` required). Advances the lifecycle field on `assignments/{assignmentId}` from `closed` back to `published` through the narrow `assignmentReopenDocRef(assignmentId).update({ status: "published" })` typed write and modifies no other field. Request payload carries only `assignmentId`; ownership fields are server-derived from the loaded record and the resolved district context. Rejects a cross-teacher or cross-school reopen with `assignments.forbidden`, and rejects a reopen from any status other than `closed` (in particular `draft` and `archived`) with `assignments.invalidTransition`. Emits a single `assignments.reopened` audit event whose payload names `{ classId }`. Idempotent under an already-`published` target: no second write and no second audit event are emitted and `alreadyPublished: true` is returned. Never mutates recipients (`assignments/{assignmentId}/recipients/*`), attempts, sessions, summaries, rollups, answer keys, ownership fields, `lessonSlug`, `lessonVersion`, `mode`, LMS linkage, or Google Classroom coursework. Together with `assignmentsClose`, defines the bidirectional `published` <-> `closed` segment of the assignment lifecycle. Sprint 13E.

**assignmentsTeacherList** (Callable). Active teacher only (authenticated caller resolved through `requireDistrictContext`; `role === "teacher"` required). Read-only enumeration of the caller's own assignments returning the minimal Curriculum-restoration projection `{ assignmentId, lessonSlug, title, classId, className, status }`. Ownership is enforced by filtering `teacherId == context.uid` and `schoolId == context.schoolId` on the `assignments` collection; the class ownership lookup on `classes/{classId}` re-verifies the class is owned by the same authenticated teacher in the same school before `className` is projected, and a mismatch quietly excludes the record. Cross-owner, cross-school, and cross-district records are excluded defensively even if a stale index returns them. Client-supplied identifiers (uid, teacherId, districtId, schoolId) are ignored and never trusted. Request payload: `{ includeDrafts?: boolean }`. Default (field omitted or `false`): the callable returns `published` and `closed` records exactly as Sprint 13C certified and excludes `draft`. Opt-in (`includeDrafts: true`, Sprint 13F): the callable widens its server-side status `in` clause to also return `draft` records owned by the same teacher in the same school as the authenticated caller; every other filter (ownership, district, class ownership) is unchanged, and another teacher's drafts are never returned. `archived` is always excluded per Data Model §3.6. No student, recipient, attempt, session, submission, rollup, or raw assessment field is projected under either mode; the aggregate-only Sprint 12E Slice 1 confidentiality boundary is preserved. This callable is a read-only enumeration and emits no per-item audit event; a single info-level `assignments.teacherList` observability log line is written per call, carrying `{ actor, count, includeDrafts }`. Sprint 13C introduced the callable; Sprint 13F extended the request contract additively with `includeDrafts` without introducing a second callable, a second registry, a Firestore Rules relaxation, a composite index, or any schema change. Sprint 15 additively extended the response projection with an optional `publishedAt` (epoch-ms number for `published` and `closed` records; `null` for `draft`) sourced from the canonical `assignments/{assignmentId}.publishedAt` field first written by `assignmentsPublish` on the initial `draft` -> `published` transition; pre-Sprint-15 clients ignore the new field. Sprint 13C, extended Sprint 13F, extended Sprint 15.

**assignmentsRecipientList** (Callable). Active teacher only (authenticated caller resolved through `requireDistrictContext`; `role === "teacher"` required). Read-only enumeration of the frozen recipient population of one owned assignment. Ownership is enforced by loading `assignments/{assignmentId}` and asserting `teacherId === context.uid` and `schoolId === context.schoolId`; a cross-teacher or cross-school target is rejected with `assignments.forbidden`. Reads `assignments/{assignmentId}/recipients/*` and defense-in-depth-filters each record against the authenticated caller's `schoolId` and `districtId` and the canonical `status === "assigned"` per PDR-029l. Display names are resolved through the canonical roster display-name resolver (`ROSTER_DISPLAY_NAME_IMPLEMENTATION_CONTRACT.md`); no denormalized personal data is written into recipient records. Response shape: `{ assignmentId, recipients: Array<{ studentId, studentDisplayName }> }`. No attempt, session, submission, score, answer, item-result, or rollup field is projected under any mode; the callable is authorized under PDR-029o and is not an aggregate analytics surface. Emits one info-level `assignments.recipientList` observability log line per call carrying `{ actor, assignmentId, count }`. Sprint 15.
